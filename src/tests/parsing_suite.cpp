...
namespace s21 {
    ...
    TEST(parsing_suite, parse_wrong_unaries) {
        ...
        PolishEvaluation evaluation;
        ASSERT_THROW(std::list<std::string> my_res = evaluation.parse_line(line), std::invalid_argument);
    }

    TEST(parsing_suite, parse_correct_numbers) {
        std::string line = "12.1 * 13 / 14.15 + 1.0";
        std::list<std::string> correct_res  = {"12.100000", "*", "13.000000", "/", "14.150000", "+", "1.000000"};
